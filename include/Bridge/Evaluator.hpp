...
class Evaluator
{
  const Eigen::Matrix<T, Eigen::Dynamic, 1, Eigen::ColMajor, 12> _values;

public:
  explicit Evaluator(std::initializer_list<T> list) : _values{ list } {}

  T operator()(Holding holding) const
  {
    T points {};

    for (decltype(_values.size()) i = 0; i < _values.size(); ++i)
      points += _values.coeff(i) * holding.test(14 - i);
